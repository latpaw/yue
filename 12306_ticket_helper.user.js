...
function injectDom() {
	...
	html.push('<p>12306 订票助手(猎豹浏览器专版)是一款用于订票的助手软件，专门为猎豹浏览器定制，无需注册。</p>');
	html.push('<p style="color: red;">助手<strong>完全免费</strong>，无需付费使用，仅接受捐助。<a href="http://www.fishlee.net/soft/44/donate.html" target="_blank" style="font-weight:bold;">捐助作者</a></p>');
	html.push('<p style="color: blue;">回家是一个单纯而简单的心愿，希望我们不会变得太复杂……</p>');
...
var utility = {
	...
	checkCompatible: function () {
		...
		if (sv != window.compVersion) {
			...
			if (!istop) {
				$("body").prepend("<div style='opacity:0.9;z-index:999; position:fixed; left:-375px; top:0px; width: 750px;margin-left:50%; color:#8A0023;border:1px solid #8A0023;line-height: 20px;background: -webkit-linear-gradient(#FFE4EA, #FFC3D1);background: -moz-linear-gradient(#FFE4EA, #FFC3D1);padding: 5px;'>助手已经检测到12306网站已更新，由于您当前使用的版本，尚未针对此次更新测试，请务必在正式购票前测试。如有任何异常请尝试使用IE浏览器订票。</div>");
			}
		} else {
			$(".versionWarning").hide();
		}
...
function initTicketQuery() {
	...
	form.find("tr:last").after("<tr class='append_row'><td colspan='9' id='queryFunctionRow'><label title='勾选此选项的话，每次你查询后，助手会帮你把始发站、到达站、日期等进行记录，下次进入查询页面后，将会帮您自动填写好'><input type='checkbox' id='keepinfo' checked='checked' />记住信息</label> <label title='勾选此选项后，假定查询的结果中没有符合你要求的车次，那么助手将会自动进行重新查询'><input checked='checked' type='checkbox' id='autoRequery' style='padding:0;' />自动重新查询</label>，每隔 <input style='width:40px;text-align:center;' type='number' min='5' value='5' size='4' id='refereshInterval' style='text-align:center;' />秒(最低5) " +
		...
		"<span style='font-weight:bold;margin-left:10px;color:#ff2020;' title='以服务器时间为准，未获得服务器时间之前，此选项不可用。启用智能加速模式时，在非正点附近时（大于0小于59分）按照正常速度刷新；当在正点附近时（大于等于59分时），暂停刷新并等到正点即刻刷新。'><label><input disabled='disabled' type='checkbox' id='chkSmartSpeed' />整点放票时刷票</label></span>" +
		"</td></tr>" +
...
<div style='color:#8A0023;line-height: 20px;background: -webkit-linear-gradient(#FFE4EA, #FFC3D1);background: -moz-linear-gradient(#FFE4EA, #FFC3D1);padding: 5px;'>请注意: 点击预订后，订单提交过快可能会出现提示验证码错误的情况，建议延迟6秒以上提交订单。<br /><strong>严重提醒！请务必开多个浏览器一起刷票啊！因为每个浏览器出现票的结果都是不一样的啊！！！！</strong></div>\
...
if (location.pathname == "/otsweb/" || location.pathname == "/otsweb/main.jsp") {
	if (isFirefox) {
		...
		unsafeInvoke(function () {
			$("body").append('<iframe id="checkVersion" width="0" height="0" style="visibility:hidden;" src="http://static.liebao.cn/content/scriptProxy.html?script=http://static.liebao.cn/content/images/apps/cn12306/checkVersion.js&spec=liebao&v=' + window.helperVersion + '"></iframe>');
		});
	}
}
